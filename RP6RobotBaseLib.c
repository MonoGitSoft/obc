 ...
 *			updateStatusLEDs();
 *			// this clears all LEDs ands the LEDs STATUS1,
 *			// STATUS6 and STATUS4!
 *
 *			// Other possibility:
...
// Distance deadrkoding
volatile uint16_t mleft_dist;
volatile uint16_t mright_dist;

volatile uint16_t mleft_dist_pose = 0;
volatile uint16_t mright_dist_pose = 0;

volatile uint16_t mleft_abs_dist = 0;
volatile uint16_t mright_abs_dist = 0;
 ...
 */
 ...
 volatile uint16_t mleft_my_speed = 0;
 volatile uint16_t mright_my_speed = 0;
ISR (INT0_vect)
{
	mleft_dist++;
	mleft_counter++;
...
//#define CHANGE_DIRECTION_FAST
      ...
      // than the original version.
			...
			// Right motor speed control: el van csuszva kamuuuuuuuuu PI reguéator
...
volatile uint16_t moved_before_dist;
